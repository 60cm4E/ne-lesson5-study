...
export default function WordArrangePage() {
  ...
  useEffect(() => { if (item) { setAvailableWords([...item.words]); setSelectedWords([]); setIsChecked(false); setIsCorrect(false); setShowHint(false); } }, [currentIndex, item, activeTab]);
  ...
  const handleCheck = () => { const c = selectedWords.join(" ").trim().toLowerCase() === item.answer.trim().toLowerCase(); setIsCorrect(c); setIsChecked(true); if (c) setScore(s => s + 1); setShowHint(true); };
    ...
    return (
      <div className="flex flex-col min-h-screen px-5 py-6 animate-fade-in">
        <header className="flex items-center justify-between mb-6">
          <Link href="/" className="text-rose-500 font-semibold text-sm">← 메인</Link>
          <h1 className="font-bold text-slate-700">{passage.title} 결과</h1>
          <div className="w-12" />
        </header>
        <div className="bg-gradient-to-br from-rose-500 to-pink-600 rounded-3xl p-8 text-center text-white shadow-xl mb-6">
        ...
        <div className="mt-auto pb-4">
          <button onClick={() => handleTabChange(activeTab)} className="w-full h-14 bg-gradient-to-r from-rose-500 to-pink-600 text-white rounded-2xl font-bold text-lg shadow-lg active:scale-[0.97]">다시 하기</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <header className="px-5 pt-4 pb-2 flex items-center justify-between">
        ...
        <span className="text-xs font-bold bg-rose-100 text-rose-600 px-3 py-1 rounded-full">{currentIndex + 1} / {total}</span>
      </header>
      ...
      <div className="flex gap-2 px-5 mb-3 hide-scrollbar overflow-x-auto">
        {sentencePassages.map((p, i) => (
          <button key={i} onClick={() => handleTabChange(i)}
            className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-all ${activeTab === i ? 'bg-rose-500 text-white shadow-md' : 'bg-slate-100 text-slate-500'}`}>
            {p.title}
          </button>
        ))}
      </div>
      ...
      <div className="mx-5 h-1.5 bg-slate-100 rounded-full mb-4 overflow-hidden">
        <div className="h-full bg-gradient-to-r from-rose-500 to-pink-500 rounded-full transition-all duration-500" style={{ width: `${(currentIndex/total)*100}%` }} />
      </div>

      <main className="flex-1 px-5 pb-28 animate-fade-in" key={`${activeTab}-${currentIndex}`}>
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs font-bold text-slate-400">해석 힌트</p>
          <button onClick={() => setShowHint(!showHint)}
            className={`text-xs font-bold px-3 py-1.5 rounded-full transition-all active:scale-95 ${showHint ? 'bg-rose-500 text-white' : 'bg-rose-100 text-rose-500'}`}>
            💡 {showHint ? '숨기기' : '힌트 보기'}
          </button>
        </div>

        <div className={`overflow-hidden transition-all duration-300 ${showHint ? 'max-h-40 opacity-100 mb-4' : 'max-h-0 opacity-0 mb-0'}`}>
          <div className="bg-rose-50 border-l-4 border-rose-400 rounded-r-xl p-4">
        ...
        <div className={`min-h-[100px] p-4 rounded-2xl border-2 border-dashed flex flex-wrap gap-2 items-start content-start mb-5 transition-all
          ${isChecked ? (isCorrect ? 'border-emerald-400 bg-emerald-50' : 'border-rose-400 bg-rose-50') : 'border-rose-300 bg-rose-50/30'}`}>
          ...
          {selectedWords.map((w, i) => (
            <button key={i} onClick={() => handleDeselect(w, i)} disabled={isChecked}
              className="px-3 py-2 bg-gradient-to-r from-rose-500 to-pink-500 text-white font-medium rounded-lg shadow-sm active:scale-95 transition-transform text-sm">{w}</button>
          ))}
        ...
        <div className="flex flex-wrap gap-2.5 justify-center">
          {availableWords.map((w, i) => (
            <button key={i} onClick={() => handleSelect(w, i)} disabled={isChecked}
              className="px-4 py-3 bg-white text-slate-700 font-semibold rounded-xl shadow-sm border border-slate-200 hover:border-rose-400 active:scale-95 transition-all min-h-[48px] text-sm">{w}</button>
          ))}
        </div>
      </main>
        ...
        <div className="max-w-lg mx-auto">
          {!isChecked ? (
            <button onClick={handleCheck} disabled={availableWords.length > 0}
              className="w-full h-14 bg-gradient-to-r from-rose-500 to-pink-600 text-white rounded-2xl font-bold text-lg shadow-lg disabled:opacity-40 active:scale-[0.97] transition-all">확인</button>
          ) : (
